...
static PATHEXT: LazyLock<PathExt> = LazyLock::new(|| {
    ...
    // SAFETY: We're going to return a self-referential struct, which is always bodgy in Rust.
    // This is safe only because this works closely together with `has_executable_extension`.
